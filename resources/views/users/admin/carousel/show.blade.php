<x-app-layout>
    <div class="main-screen">
        <x-admin.sidebar-new />
        <div class="main-content">
            <x-admin.navbar-new />

            <div class="panel min-h-screen" x-data="carouselShow" x-init="activeToggleInit({{$carousel->active}})">
                <h1 class="page-title">Carousel - show</h1>
                <div class="flex justify-between">
                    <a href="{{ route('admin.imageCarousel.index') }}" class="btn-generic"><i
                            class="fi fi-rr-arrow-small-left pt-1"></i></a>



                    <div>
                        <label class="relative inline-flex items-center cursor-pointer">
                            <input id="isActiveToggle" type="checkbox" @change="activeToggleAction($event)" class="sr-only peer">
                            <div
                                class="w-11 h-6 bg-gray-200 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-blue-300 dark:peer-focus:ring-blue-800 rounded-full peer dark:bg-gray-700 peer-checked:after:translate-x-full rtl:peer-checked:after:-translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:start-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all dark:border-gray-600 peer-checked:bg-blue-600">
                            </div>
                            <span class="ms-3 text-sm font-medium text-gray-900 dark:text-gray-300">Active</span>
                        </label>
                    </div>
                </div>
                <div class="flex gap-2 h-full">
                    <div class="w-1/2 h-96">
                        <img src="{{ $carousel->image }}" alt="" srcset=""
                            class="object object-center h-full w-full">
                    </div>

                    <div class="w-1/2 h-96">
                        <h1 class="text-xl font-bold py-5">
                            {{ $carousel->title }}
                        </h1>
                        <p class="text-sm whitespace-pre-line">
                            {{ $carousel->description }}
                        </p>
                    </div>

                </div>
            </div>
        </div>
    </div>
</x-app-layout>
